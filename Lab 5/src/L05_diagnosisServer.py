...
class Server:
    ...
    def handle_message(self, command):
        if command.startswith('dtc-P0138'):
            ...
            self.send_bytes_to_client(
                "display-popup#Oxygen Sensor malfunction. Your can still drive your car for a short time but your vehicle fuel economy will suffer. Serice your car as soon as possible.")

        if command.startswith('dtc-P0300'):
            self.send_bytes_to_client(
                "display-popup#Random, Multiple Misfire Detected. This is a critical issue. You need to stop driving immediately and tow your car to the nearest authorized service.")
            self.send_bytes_to_client("service-soon")

        if command.startswith('dtc-P0700'):
            self.send_bytes_to_client(
                "display-popup#Transmission Control System Malfunction. Please stop driving the vehicle immediately.")
            self.send_bytes_to_client("service-soon")
