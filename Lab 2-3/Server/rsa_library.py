...
def encrypt(public_key, hex_number):
    return str(int(hex_number) ** public_key[0] % public_key[1])


# ############################## EXERCISE 2 ###############################
def decrypt(private_key, encrypted_msg):
    return str(int(encrypted_msg) ** private_key[0] % private_key[1])
...
def number_check(hex_nr):
    ...
    """
    status = hex(int('0x' + hex_nr[-2:], 16) + int(hex_nr[:4], 16))
    if status == '0xff':
